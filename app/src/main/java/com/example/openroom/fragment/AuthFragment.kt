...
class AuthFragment : Fragment(R.layout.authorization_fragment) {
    ...
    private fun signIn() {
        ...
        val agent = Agent(null, null, null, phone, null, password, null, null, null, null)
        apiManager!!.signInAgent(agent, object : Callback<ResponseBody?> {
            override fun onResponse(call: Call<ResponseBody?>, response: Response<ResponseBody?>) {
                when (response.code()) {
                    ...
                    406 -> {
                        Toast.makeText(context, "Пароль или логин не совпадают", Toast.LENGTH_SHORT)
                            .show()
                    }
                }
            }
